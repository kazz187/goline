...
func TestProviderRegistration(t *testing.T) {
	...
	// Check provider name
	if p.Name() != "deepseek" {
		t.Errorf("Expected provider name to be 'deepseek', got '%s'", p.Name())
	}
...
func TestModelSelection(t *testing.T) {
	testCases := []struct {
		...
		{"Valid model name", string(DeepSeekReasoner), DeepSeekReasoner},
...
func TestProviderFactory(t *testing.T) {
	...
	factory, ok := provider.GetFactory("deepseek")
	if !ok {
		t.Fatal("DeepSeek provider factory not registered")
	}
